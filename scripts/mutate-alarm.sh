...
"; then
    ...
    # rapor üretilir. Bu betikte tam olarak bu oldu: dört kenar
    # tetikleme mutasyonu `if fresh {` → `if true {` diyordu,
    # "fresh declared and not used" ile derleme düşüyordu ve dördü de
    # sahte "yakalandı" veriyordu.
    #
    # Derleme ayrı ölçülünce bu sınıf tamamen kapanıyor: kırmızı artık
    # yalnızca testin iddiasından gelebilir.
    if ! go build "$pkg" >/dev/null 2>&1; then
        echo "  !! MUTANT DERLENMİYOR: $name — ölçüm YAPILMADI, "
        echo "     mutasyon derlenebilir olacak şekilde yazılmalı"
        fail=1
        return
    fi
